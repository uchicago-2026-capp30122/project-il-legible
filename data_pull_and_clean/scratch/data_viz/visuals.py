...
def total_donations_hist(df):
    ...
    chart = (
        ...
        .encode(
            ...
            y=alt.Y("count()", title="Number of Sponsors", axis=alt.Axis(grid=False)),
        )
        .properties(
            title=f"Total Donation Amounts of Primary Sponsors in 102nd & 103rd Sessions"
        )
    )
...
def average_donation_hist(df):
    ...
    chart = (
        ...
        .properties(
            title=f"Average Donation Amount of Primary Sponsors in 102nd & 103rd Sessions"
        )
    )
...
def num_bills_total_donations_scatter(df):
    ...
    points = base.mark_circle(size=SIZE, color=MAIN_COLOR).encode(
    ...
    chart = (
        ...
        .interactive()
        .properties(
            title="Total Donations vs Number of Bills Introduced in 102nd & 103rd Sessions"
        )
    )
...
def bills_introduced_legislator(df: pd.DataFrame, name: str) -> alt.Chart:
    ...
    legislator_df = pd.DataFrame(
        {"names": ["Median", name], "num_bills": [median_bills, legislator_bills]}
    )
    ...
    chart = (
        ...
        .properties(
            title="Bills Introduced in 102nd & 103rd Sessions", width=300, height=400
        )
    )
    ...
    base = (
        ...
        .encode(
            ...
            color=alt.Color(
                "Legend",
                scale=alt.Scale(
                    domain=["Passed", "Failed"], range=["#062F8E", "#7C7C7C"]
                ),
                legend=alt.Legend(titleFontSize=18, labelFontSize=18),
            ),
        )
        .properties(title="Bill Passage", width=200, height=250)
    )
...
def num_bills_entity_donations_scatter(df):
    ...
    base = (
        ...
        .add_params(donation_choice)
        .transform_calculate(
            donation_x="DonationWindow == 'All time' ? datum.amt_allcond_all : datum.amt_allcond_L3"
        )
    )
    ...
    chart = (
        ...
        .interactive()
        .properties(
            title="Entity Donations vs Number of Bills Introduced in 102nd & 103rd Sessions"
        )
    )
...
def bill_passage_pct_entity_donations_scatter(df):
    ...
    points = base.mark_circle(size=SIZE, color=MAIN_COLOR).encode(
        ...
        tooltip=[
            alt.Tooltip("name", title="Legislator"),
            alt.Tooltip(
                "pct_c_allcond_all", title="All-time Entity Donation Percentage"
            ),
            alt.Tooltip(
                "pct_c_allcond_L3", title="Last 3 years Entity Donation Percentage"
            ),
    ...
    chart = (
        ...
        .interactive()
        .properties(
            title="Entity Donation Percentage vs Bill Passage Rate in 102nd & 103rd Sessions"
        )
    )
...
def large_donation_barchart(df, name):
    ...
    bars = base.mark_bar(color=MAIN_COLOR).encode(
        x=alt.X(
            "pct_selected:Q",
            title="",
    ...
    labels = base.mark_text(align="left", dx=3).encode(
        x="pct_selected:Q",
        y="category:N",
    ...
    chart = (bars + labels).properties(
        title=f"Percentage of Large vs Small Donations for {name}"
    )
...
def entity_donation_barchart(df, name):
    ...
    chart = (bars + labels).properties(
        title=f"Percentage of Entity vs Individual Donations for {name}"
    )
...
def in_state_donation_barchart(df, name):
    ...
    chart = (bars + labels).properties(
        title=f"Percentage of In-State vs Out-of-State Donations for {name}"
    )

...
def aggregate_bill_stats(bills: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    sponsor_1 = (
        bills.groupby("primary_sponsor_1_clean")
        .agg(
            {
    ...
    sponsor_2 = (
        bills.groupby("primary_sponsor_2_clean")
        .agg(
            {
                "organization_classification": "first",
                "id": "count",
    ...
    df = sponsor_1.merge(
        sponsor_2,
        how="left",
        left_on="primary_sponsor_1_clean",
    ...
    bill_agg["bills_passed"] = (
        df["passed_full_legislature_x"] + df["passed_full_legislature_y"]
    )
...
def calculate_legislator_effectiveness(sponsors: pd.DataFrame) -> pd.DataFrame:
    ...
    sponsors["effectiveness_score"] = (
        sponsors["bills_introduced_percentile"] + sponsors["passage_rate_percentile"]
    ) / 2
...
def main():
    donations = pd.read_csv(
        "data_pull_and_clean/pull_IL_sunshine/intermediate_data/donation_stats.csv"
    )
    bills = pd.read_csv(

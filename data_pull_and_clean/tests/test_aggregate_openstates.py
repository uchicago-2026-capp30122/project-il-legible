...
from data_pull_and_clean.pull_open_states.get_bulk_data import get_actions, get_sponsors
from data_pull_and_clean.pull_open_states.aggregate_openstates import (
    summarize_actions,
    summarize_sponsors,
)
from data_pull_and_clean.pull_open_states.clean_name_column import general_name_cleaning
from data_pull_and_clean.pull_open_states.clean_name_column import general_name_cleaning
...
def test_summarize_actions_values(get_actions_sample):
    ...
    assert (
        actions_summary.loc["ocd-bill/002a4c9d-e0bc-4f58-8f93-4f6656cf73d5"][
...
def test_summarize_sponsors_values(get_sponsors_sample):
    ...
    sponsors_summary = summarize_sponsors(get_sponsors_sample)
    assert (
        sponsors_summary.loc["ocd-bill/00a8be44-1eda-4f58-9a89-26a72911b601"][
            "num_sponsors"
        ]
        == 14
    )
...
def test_automated_name_cleaning():
    ...
    assert (
        general_name_cleaning('Christopher "C.D." Davidsmeyer')

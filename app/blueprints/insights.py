...
def index():
    ...
    total_count_donations = sum(
        s.donation_count_all for s in sponsors if s.donation_count_all is not None
    )
    ...
    summary_stats = {
        ...
        "last_three_donations": sum(
            s.total_L3 for s in sponsors if s.total_L3 is not None
        ),
        "average_donation": total_donations / total_count_donations,
    }

    charts = {
        "total_donation_history": viz.total_donation_history(sponsor_query)
        .configure_view(strokeWidth=0)
        .to_json(),
        "average_donation_history": viz.average_donation_history(sponsor_query)
        .configure_view(strokeWidth=0)
        .to_json(),
        "bills_by_donations": viz.bills_by_donations_scatter(sponsor_query).to_json(),
        "sankey": viz.bill_progress_sankey(bill_query).to_html(
            full_html=False, include_plotlyjs="cdn"
        ),
        "passing_by_entities": viz.bill_passage_pct_entity_donations_scatter(
            sponsor_query
        ).to_json(),
    }

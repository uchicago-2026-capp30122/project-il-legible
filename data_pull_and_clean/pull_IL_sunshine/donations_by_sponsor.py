...
def calculate_summary_stats(
    ...
    # Large donations
    row["pct_c_above_all"] = safe_divide(
        (df["amount"] > LARGE_DONATION).sum(), total_len
    )
    row["pct_c_above_L3"] = safe_divide(
        (df["last_3_years"] & (df["amount"] > LARGE_DONATION)).sum(), L3_len
    )
    ...
    row["pct_c_allcond_L3"] = safe_divide(
        (entity_mask & df["last_3_years"]).sum(), L3_len
    )

    # In-state (Illinois) vs. out-of-state donations
    row["pct_c_IL_all"] = safe_divide((df["state"] == "IL").sum(), total_len)
    row["pct_c_IL_L3"] = safe_divide(
        (df["last_3_years"] & (df["state"] == "IL")).sum(), L3_len
    )
...
def main():
    ...
    with open(
        "data_pull_and_clean/pull_IL_sunshine/intermediate_data/donation_stats.csv", "w"
    ) as file:

...
def total_donation_history(sponsors):
    ...
    chart = (
        ...
        .transform_bin(
            ["bin_start", "bin_end"], field="total_all", bin=alt.Bin(maxbins=50)
        )
        .encode(
            x=alt.X(
                "bin_start:Q",
            ...
            y=alt.Y(
                ...
                axis=alt.Axis(
                    grid=False, labelFontSize=12, titleFontSize=15, titlePadding=10
                ),
            ),
            tooltip=[
                alt.Tooltip("count()", title="Number of Sponsors", format=","),
                alt.Tooltip(
                    "bin_start:Q", title="Total Donations From", format="$,.0f"
                ),
...
def num_bills_bar(sponsor_name: str, sponsors):
    ...
    chart = (
        ...
        .encode(
            x=alt.X(
                "names",
                title="",
                axis=alt.Axis(labelAngle=0, labelFontSize=15, grid=False),
            ),
            y=alt.Y("num_bills", title="", axis=alt.Axis(labelFontSize=12, grid=False)),
            tooltip=[
    ...
    text = chart.mark_text(dy=-10, fontSize=17, color=COLORS["blue"]).encode(
    ...
    base = (
        ...
        .encode(
            ...
            color=alt.Color(
                "Legend",
                scale=alt.Scale(
                    domain=["Passed", "Failed"], range=[COLORS["blue"], COLORS["grey"]]
                ),
                legend=None,
            ),
            tooltip=[
...
def average_donation_history(sponsors):
    ...
    chart = (
        alt.Chart(df)
        .transform_bin(
            ["bin_start", "bin_end"], field="avg_donation_all", bin=alt.Bin(maxbins=50)
        )
        .mark_bar(color=COLORS["red"])
        .encode(
            x=alt.X(
                ...
                axis=alt.Axis(
                    format="~s", labelFontSize=12, titleFontSize=15, titlePadding=15
                ),
            ),
            x2="bin_end:Q",
            y=alt.Y(
            ...
            tooltip=[
                alt.Tooltip("count()", title="Number of Sponsors", format=","),
                alt.Tooltip(
                    "bin_start:Q", title="Average Donation From", format="$,.0f"
                ),
                alt.Tooltip("bin_end:Q", title="To", format="$,.0f"),
            ],
        )
...
def bills_by_donations_scatter(sponsors):
    ...
    donation_choice = alt.param(
        ...
        bind=alt.binding_select(
            options=["All Time", "Last 3 Years"], name="Donations: "
        ),
    )
    ...
    base = (
        ...
        .transform_calculate(
            donation_x="DonationWindow == 'All Time' ? datum.total_all : datum.total_L3"
        )
    )

    points = base.mark_circle(size=SIZE, color=COLORS["blue"]).encode(
        x=alt.X(
            "donation_x:Q",
            title="Total Donation Amount",
            axis=alt.Axis(
        ...
        y=alt.Y(
            "num_bills",
            title="Number of Bills Introduced",
            axis=alt.Axis(
                grid=False, labelFontSize=12, titleFontSize=15, titlePadding=10
            ),
        ),
    ...
    line = (
        base.transform_regression("donation_x", "num_bills")
        .mark_line(color="#A5A5A5", strokeDash=[5, 5])
        .encode(x="donation_x:Q", y="num_bills:Q")
    )

    chart = (
        (points + line)
        .properties(width="container")
        .transform_calculate(url="/sponsors/" + alt.datum.id)
        .interactive()
    )
...
def large_donation_barchart(name, sponsors, time):
    ...
    chart_df = pd.DataFrame(
        {
            ...
            "count_all": [
                ...
                int(
                    (1 - float(person["pct_c_above_all"]))
                    * person["donation_count_all"]
                ),
            ],
            "count_L3": [
                int(person["pct_c_above_L3"] * person["donation_count_L3"]),
                int(
                    (1 - float(person["pct_c_above_L3"])) * person["donation_count_L3"]
                ),
            ],
        }
    )
    ...
    bars = (
        ...
        .encode(
            ...
            y=alt.Y(
                "category:N",
                title="",
                axis=alt.Axis(ticks=False, labelFontSize=15, labelPadding=10),
            ),
            tooltip=[
    ...
    labels = (
        alt.Chart(chart_df)
        .mark_text(align="left", dx=3, fontSize=15, color=COLORS["red"])
        .encode(
            x=pct_selected + ":Q",
            y="category:N",
...
def bill_progress_sankey(bills):
    ...
    for i, val in enumerate(labels):
        node_text.append(
            f"{val}<br>{values_all[i]:,} bills<br>{(values_all[i] / n_introduced):.0%} of total"
        )
    node_text[1] = ""
    ...
    fig.update_layout(
        font_size=15, width=2000, height=400, xaxis=dict(rangeslider=dict(visible=True))
    )
...
def bill_passage_pct_entity_donations_scatter(sponsors):
    ...
    base = (
        ...
        .add_params(donation_choice)
        .transform_calculate(
            donation_x="DonationWindow == 'All Time' ? datum.pct_c_allcond_all : datum.pct_c_allcond_L3"
        )
    )
    ...
    points = base.mark_circle(size=SIZE, color=COLORS["blue"]).encode(
        x=alt.X(
            "donation_x:Q",
            title="Percentage of Donation Count from Entities",
            axis=alt.Axis(
                grid=False,
                labelFontSize=12,
                titleFontSize=15,
        ...
        tooltip=[
            ...
            alt.Tooltip(
                "pct_c_allcond_all",
                title="All Time Percent of Donations from Entities",
                format=".0%",
            ),
            alt.Tooltip(
                "pct_c_allcond_L3",
                title="Last 3 Years Percent of Donations from Entities ",
                format=".0%",
            ),
    ...
    line = (
        base.transform_regression("donation_x", "pct_bills_passed")
        .mark_line(color="#A5A5A5", strokeDash=[5, 5])
        .encode(x="donation_x:Q", y="pct_bills_passed:Q")
    )

...
def clean_sponsor_names(bills: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in sponsor_cols:
        bills[f"{col}_clean"] = (
            bills[f"{col}"]
            .fillna("")
            .apply(general_name_cleaning)
            .apply(specific_name_changes)
        )
...
def output_unique_sponsors_csv(bills_cleaned: pd.DataFrame) -> None:
    ...
    Outputs:
        None. Directly saves a CSV
    """
    unique_sponsors = pd.DataFrame(
        bills_cleaned["primary_sponsor_1_clean"].unique(), columns=["Sponsor"]
    )

    unique_sponsors_out_path = Path(
    ...
    unique_sponsors.to_csv(unique_sponsors_out_path)
...
def main():
    filepath = Path(
        "data_pull_and_clean/pull_open_states/intermediate_data/openstates_cleaned"
    )
    bills = pd.read_csv(filepath)
    bills_cleaned = clean_sponsor_names(bills)

    # Save CSV with full dataset before tirmming down to final versions
    bills_cleaned.to_csv(
        "data_pull_and_clean/pull_open_states/intermediate_data/openstates_w_names.csv"
    )
